...
io.sockets.on('connection', function(socket) {
	...
	if (usableRooms.length > 0) {
		...
		for (var i = 0; i < usableRooms.length; i++) {
			...
			if (usableRooms[i].roomUsers.length < parseInt(usableRooms[i].roomMaxSize) && usableRooms[i].roomName == data.roomName && usableRooms[i].roomPassword == data.roomPassword && usableRooms[i].roomReady < parseInt(usableRooms[i].roomMaxSize)) {
					//
